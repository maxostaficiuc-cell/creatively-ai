...
import { getConnectedAdAccount, fetchMetaAds } from "@/lib/meta/client";
...
export default async function Page() {
  ...
  if (account?.access_token && account.external_account_id) {
    try {
      rows = (await fetchMetaAds(account.access_token, account.external_account_id)) as Row[];
    } catch {
  ...
  return (
    <AppShell profile={profile} greeting="Ads" subtitle="">
      {!account ? (
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-dashed border-base-border bg-base-surface/40 p-10 text-center text-sm text-ink-secondary">
          No ads found on your connected Meta account yet.
        </div>
      ) : (
