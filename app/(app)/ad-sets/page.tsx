...
import { getConnectedAdAccount, fetchMetaAdSets } from "@/lib/meta/client";
...
export default async function Page() {
  ...
  if (account?.access_token && account.external_account_id) {
    try {
      rows = (await fetchMetaAdSets(account.access_token, account.external_account_id)) as Row[];
    } catch {
  ...
  return (
    <AppShell profile={profile} greeting="Ad Sets" subtitle="">
      {!account ? (
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-dashed border-base-border bg-base-surface/40 p-10 text-center text-sm text-ink-secondary">
          No ad sets found on your connected Meta account yet.
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-wide text-ink-muted">
                <th className="px-4 py-3 font-medium">Ad Sets</th>
